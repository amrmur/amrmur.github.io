...
window.onload = function(){
    ...
    document.addEventListener("click", moveDoodler2);
}
const targetFPS = 60;
...
function update() {
    ...
    if (elapsed >= frameInterval) {
        ...
        velocityY = Math.min(velocityY + gravity, 16);
        doodler.y += velocityY;
...
function moveDoodler2(e) {
    if(gameOver){
        ...
        gameOver = false;
        placePlatforms;
    }else if(e.clientX > viewportWidth/2) { // move right
        velocityX = 4;
        doodler.img = doodlerRightImg;
    } else { // move left
        velocityX = -4;
...
function placePlatforms(){
    ...
    for(let i = 0; i < 5; i++){
        let randomX = Math.floor(Math.random() * boardWidth*5/6); // (0-1) * boardWidth*5/6
        platform = {
            img: platformImg,
            x: randomX,
            y: boardHeight - 120*i - 150,
            width: platformWidth,
            height: platformHeight
        }
        platformArray.push(platform);
